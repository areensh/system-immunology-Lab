...
**Figure 17.** Within-individual clone size across tissues.

**Figure 18.** Within-individual tissue comparison (paired line plots).
...
The within-individual analysis (**Figure 16**) showed that clone counts vary substantially across tissues within the same healthy donor, with peripheral blood (PBL) typically harboring the most clones. The cross-study gut tissue comparison (**Figures 17–18**) revealed differences in clone size distributions between healthy donors and post-transplant patients in shared gut tissues (Colon, Jejunum, MLN), consistent with the distinct immunological environments of native versus post-transplant gut mucosa. The ability to make such comparisons across independently built databases — without manual data harmonization — demonstrates the practical utility of IS-API for multi-study immune repertoire analysis.
...
**V gene usage is relatively stable across disease states.** Hierarchical clustering of the 16 most prevalent V genes by their mean frequency profiles across disease categories (**Figure 11**) revealed two distinct groups: a high-frequency cluster (IGHV3-23, IGHV4-59, IGHV4-34, IGHV4-39) that dominates the repertoire across all conditions, and a lower-frequency cluster comprising the remaining 12 genes. Within the lower-frequency group, V genes clustered by family — IGHV1 family members (IGHV1-18, IGHV1-2, IGHV1-69) grouped together, as did several IGHV3 family genes (IGHV3-15, IGHV3-48, IGHV3-33, IGHV3-7) — suggesting that co-regulation or shared genomic features drive correlated usage patterns. Importantly, the clustering was driven by overall expression level rather than by disease-specific differences, as no V gene cluster showed a disease-associated usage shift. This stability is consistent with findings from large-scale repertoire studies showing that V gene usage is primarily determined by germline genetics rather than by antigen exposure or disease state [37, 6]. While specific antibody lineages targeting SARS-CoV-2 epitopes may preferentially use certain V genes [38], these represent a small fraction of the total repertoire and may not be detectable at the level of overall V gene frequency.
