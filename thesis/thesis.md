...
We first examined clone counts across the seven HC1 tissues for each healthy donor individually (**Figure 16**). We then compared clone size distributions between HC1 and GT1 in shared tissues (**Figure 17**), and compared median clone sizes per tissue across all subjects from both studies (**Figure 18**).
...
![Figure 17. Clone size distributions in shared tissues — HC1 donor D207 vs. GT1 donor. Each panel shows one tissue (Colon, Ileum, Jejunum, MLN); violin and box plots compare clone size distributions between the two individuals. Clone size is the number of unique sequences belonging to that clone within the specific tissue (not the total clone size across all tissues). Red diamonds indicate means; n = number of clones in that tissue. Y-axis: log scale.](../immunedb_STATS_API/clonal_analysis/plots/15_within_subject_clone_size_cross_study.png){ width=100% }
