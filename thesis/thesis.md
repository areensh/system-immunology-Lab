...
![Figure 16. Clone count per tissue for each HC1 healthy donor. Each subplot shows one individual; bars represent the number of distinct clones detected in each of the seven HC1 tissues (PBL, BM, SPL, Lung, Colon, Ileum, MLN). Clones are defined at the subject level by ImmuneDB, so a clone that appears in multiple tissues is counted once in each tissue where it has at least one sequence. Y-axis is uniform across all subplots.](../immunedb_STATS_API/clonal_analysis/plots/14_within_subject_clone_count.png){ width=100% }
