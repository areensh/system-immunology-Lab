...
![Figure 18. Clone count per tissue for each HC1 healthy donor. Each subplot shows one individual; bars represent the total number of clones detected in each of the eight HC1 tissues (PBL, BM, SPL, Lung, Colon, Ileum, Jejunum, MLN). These counts include all clones present in each tissue; the current API query does not distinguish tissue-exclusive clones from clones shared across tissues within the same individual. Y-axis is uniform across all subplots.](../immunedb_STATS_API/clonal_analysis/plots/14_within_subject_clone_count.png){ width=100% }
...
The within-individual analysis (**Figure 18**) showed that clone counts vary substantially across tissues within the same healthy donor, with peripheral blood (PBL) typically harboring the most clones. The cross-study gut tissue comparison (**Figures 19–20**) revealed differences in clone size distributions between healthy donors and post-transplant patients in shared gut tissues (Colon, Jejunum, MLN), consistent with the distinct immunological environments of native versus post-transplant gut mucosa. The ability to make such comparisons across independently built databases — without manual data harmonization — demonstrates the practical utility of IS-API for multi-study immune repertoire analysis.
