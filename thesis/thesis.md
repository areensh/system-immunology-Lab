...
These cross-study tissue comparisons revealed that clone counts and sizes can differ substantially between tissues in the same individual and between studies, highlighting both the importance of specifying tissue type and the value of IS-API's ability to query across databases. The observation that bone marrow and blood repertoires from the same individual can have different clonal profiles is consistent with the known compartmentalization of B cell populations [34, 41]. The ability to make such comparisons across independently built databases — without manual data harmonization — demonstrates the practical utility of IS-API for multi-study immune repertoire analysis.
