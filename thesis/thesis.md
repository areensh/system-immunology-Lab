...
*Across-disease comparison within each method.* Among 5'RACE+UMI studies only (CD1, CVX1, CVX2), the disease-severity patterns observed in the full dataset were preserved: individuals with severe disease had low clone counts and high CDR NS/S ratios, individuals with mild disease had the lowest clone counts, and COVID-naive and recovered individuals had higher clone counts and comparable CDR NS/S ratios. Similarly, among multiplex PCR studies only (CD2, CD3, HC1), individuals with severe disease had lower clone counts than individuals with moderate disease and healthy controls, and the CDR NS/S pattern — lower in the Moderate group, higher in the Severe group — was consistent with the full-cohort analysis. The fact that disease-stage trends are qualitatively preserved within each sequencing method provides evidence that the biological patterns are not solely driven by methodological differences.
