...
**Figure 10.** NS/S ratios and mutation counts — expanded vs. rest by disease stage.
...
**Figure 17.** Within-individual clone count across tissues.

**Figure 18.** Within-individual clone size across tissues.

**Figure 19.** Within-individual tissue comparison (grouped dot plot).
...
![Figure 10. Somatic hypermutation — expanded vs. rest by disease stage. (A–B) CDR and FW NS/S ratios comparing expanded clones (≥20 unique sequences, solid boxes) versus unexpanded clones (faded boxes). (C–D) median NS/S ratios as paired bar charts for CDR and FW regions. Boxplots show median and IQR; dashed line marks NS/S = 2.0 (neutral expectation). Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/21b_mutations_expanded_vs_rest.png){ width=100% }
...
**V gene usage is relatively stable across disease states.** Hierarchical clustering of the 16 most prevalent V genes by their mean frequency profiles across disease categories (**Figure 11**) revealed two distinct groups: a high-frequency cluster (IGHV3-23, IGHV4-59, IGHV4-34, IGHV4-39) that dominates the repertoire across all conditions, and a lower-frequency cluster comprising the remaining 12 genes. Within the lower-frequency group, V genes clustered by family — IGHV1 family members (IGHV1-18, IGHV1-2, IGHV1-69) grouped together, as did several IGHV3 family genes (IGHV3-15, IGHV3-48, IGHV3-33, IGHV3-7) — suggesting that co-regulation or shared genomic features drive correlated usage patterns. To directly test whether V gene usage differs between disease states, we clustered all 106 individuals by their V gene usage profiles (**Figure 12**). The resulting dendrogram showed disease categories interleaved throughout, with no separation of disease states into distinct branches, confirming that V gene usage does not discriminate between disease categories. This stability is consistent with findings from large-scale repertoire studies showing that V gene usage is primarily determined by germline genetics rather than by antigen exposure or disease state [37, 6]. While specific antibody lineages targeting SARS-CoV-2 epitopes may preferentially use certain V genes [38], these represent a small fraction of the total repertoire and may not be detectable at the level of overall V gene frequency. Additionally, because the analysis included only V genes present at ≥1% frequency in at least 85% of individuals, disease-specific V gene enrichments involving lower-frequency genes would not have been captured.
