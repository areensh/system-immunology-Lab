...
**Sequencing method (**Figure 15**).** Because the six studies used different library preparation methods — 5'RACE with UMI error correction (CD1, CVX1, CVX2) versus multiplex PCR without UMIs (CD2, CD3, HC1) — and these methods are unevenly distributed across disease categories (see Metadata Figure 5), sequencing method is a potential confounder. To assess this, we compared clone count, mean CDR3 length, and CDR NS/S ratio between sequencing methods within disease categories where both methods were represented. In the Severe group (n=11 5'RACE, n=15 PCR), clone counts were notably higher in PCR-sequenced individuals, likely reflecting the higher read depth and broader primer coverage of the CD2/CD3 studies rather than a biological difference. Mean CDR3 length and CDR NS/S ratio showed overlapping distributions between methods within the Severe and Recovered groups, suggesting that these metrics are less sensitive to sequencing methodology. Three disease categories — Moderate (PCR only), COVID Naive (5'RACE only), and Healthy (PCR only) — were each sequenced with a single method, making it impossible to disentangle disease effects from method effects in these groups. This confound is an inherent limitation of the cross-study design and should be considered when interpreting differences involving these categories.
