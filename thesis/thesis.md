...
**Metadata Figure 1.** Sampling depth across studies: number of individuals per dataset and sequencing depth per individual.
...
**Figure 1.** Clone count distribution by disease stage.

**Figure 2.** Diversity profiles by disease stage.

**Figure 3.** Expanded clone count at multiple thresholds.

**Figure 4.** Clone size distribution by disease stage.

**Figure 5.** Clone size: copies versus unique sequences by disease stage.

**Figure 6.** Clonal concentration (Top X) by disease stage.

**Figure 7.** CDR3 length distribution: all clones by disease stage.

**Figure 8.** CDR3 length distribution: expanded clones by disease stage.

**Figure 9.** NS/S ratios and mutation counts — all clones by disease stage.

**Figure 10.** NS/S ratios and mutation counts — expanded vs. unexpanded by disease stage.

**Figure 11.** V gene usage heatmap by disease stage with individuals clustered by V gene usage.

**Figure 12.** Dendrogram of individuals clustered by V gene usage profiles.

**Figure 13.** Clonal metrics versus age by disease stage.

**Figure 14.** Clonal metrics by gender and disease stage.

**Figure 15.** Clonal metrics by sequencing method and disease stage.

**Figure 16.** NS/S ratios (CDR and FW) — all clones by sex and disease stage.

**Figure 17.** NS/S ratios (CDR and FW) — expanded clones by sex and disease stage.

**Figure 18.** Clone count per tissue for each HC1 healthy donor.

**Figure 19.** Clone size comparison in healthy vs. post-transplant gut tissue.

**Figure 20.** Median clone size per subject by tissue — HC1 vs. GT1.
...
A key feature of IS-API is its use of CTE-based SQL queries (referred to as sampleMetaCTE) that ensure correct per-individual, per-tissue aggregation of statistics. This addresses a fundamental challenge in cross-study analysis: different studies may have different numbers of samples per individual and different tissue types, and naive aggregation can produce misleading results. The CTE approach first identifies the relevant samples for each individual matching the requested metadata filters, then aggregates clone-level statistics across those samples, ensuring that each individual contributes one data point regardless of their number of samples.
...
![Metadata Figure 1. Sampling depth across studies. Left panel: number of individuals per dataset (bars). Right panel: sequencing depth (number of sequences) per individual within each dataset (dots). Study IDs: CD1–CD3 (COVID-19 cohorts), CVX1–CVX2 (vaccine cohorts), HC1 (healthy controls), GT1 (gut transplant).](../immunedb_STATS_API/metadata/plots/06_sampling_depth.png){ width=100% }
...
**Library preparation and sequencing differences.** The IS-API can also expose how the studies differ in their library preparation and sequencing methodologies, by querying relevant metadata fields. CD1 used 5'RACE (rapid amplification of cDNA ends) with unique molecular identifiers (UMIs) on the Illumina MiSeq platform, providing error-corrected sequences. CD2 and CD3 used multiplex PCR-based amplification of heavy chain variable regions, also sequenced on Illumina platforms, but without UMI-based error correction. The vaccine studies (CVX1, CVX2) used 5'RACE with UMIs, similar to CD1. HC1 used multiplex PCR amplification sequenced on Illumina HiSeq. All studies were annotated with IgBLAST for germline assignment, and clonal clustering was performed within ImmuneDB using shared V gene, J gene, and CDR3 similarity criteria. While IS-API standardizes the downstream analysis, these upstream differences in library preparation and error correction could introduce systematic biases — particularly in mutation rate estimates, where PCR errors without UMI correction may inflate apparent mutation counts. This limitation is acknowledged in the Discussion.

**Study selection and exclusions.** Based on the metadata overview, we excluded GT1 (n=15) from the disease-stage analysis because it does not include disease stage as a metadata field. This left six datasets with 106 individuals.
...
- Individuals with severe and mild disease span a wide age range (20-88 years), while recovered and COVID-naive individuals tend to be younger (20-50 years).
...
**Clone count by disease stage.** IS-API's clone_count endpoint returns the total number of unique clones per individual. The clone count distribution (**Figure 1**) revealed highly significant differences across disease categories (Kruskal-Wallis p < 0.0001). Individuals with mild disease had the fewest clones (median 2,228), while recovered (median 22,177), healthy (median 20,735), and COVID-naive (median 16,682) individuals had substantially higher clone counts. Individuals with severe (median 5,089) and moderate (median 6,184) disease fell between these extremes. The low clone counts in the Mild group, combined with the high variability spanning two orders of magnitude (from ~100 to ~6,000), suggest that Mild COVID-19 may be associated with more restricted repertoire sampling or reduced clonal diversity. However, an important caveat is that sequencing depth varies substantially across studies and individuals (see **Metadata Figure 1**), and this variation is not uniform across disease categories. For example, within the Healthy group alone, clone counts range from 650 to 61,136 — the upper extreme belongs to HC1 individual D207, who is known to be deeply sampled. Since clone count is directly influenced by how many sequences are captured per individual, differences in sequencing depth could partly explain the observed variation between disease groups, particularly for categories dominated by a single study. Without subsampling to a uniform depth, clone count comparisons should be interpreted with this limitation in mind. The consistently high clone counts in recovered, healthy, and COVID-naive individuals indicate that these groups maintain broader repertoire diversity, though the contribution of higher sequencing depth in those studies cannot be fully separated from a biological effect.

![Figure 1. Clone count distribution by disease stage. Total number of unique clones per individual (log scale) across six disease categories. Each dot represents one individual; boxplots show median and IQR. Kruskal-Wallis p-value annotated. Colors: Severe (red), Moderate (orange), Mild (salmon), Recovered (green), COVID Naive (light blue), Healthy (blue). Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/27_clone_count_by_disease.png){ width=100% }

**Clonal diversity profiles.** To assess clonal diversity, we computed Hill numbers of orders 0 (richness), 1 (Shannon), and 2 (Simpson) from the clone size distribution of each individual. The diversity profiles (**Figure 2**) plot all three Hill numbers for each individual with group medians highlighted, where each line slopes downward from q0 to q2 — a steeper negative slope indicates greater dominance by a few large clones. All three diversity measures showed significant differences across disease categories (Kruskal-Wallis p < 0.001 for all three orders). The most striking pattern was a clear separation between two groups: COVID-naive and recovered individuals had the highest absolute diversity across all three orders (median q0 of ~350 and ~170, respectively), while individuals with active or recent disease — Severe, Moderate, and Mild — clustered at much lower diversity levels (median q0 of ~15, ~20, and ~4, respectively). Healthy individuals fell between these groups (median q0 ~65). The slope of each profile — quantified by the ratio of Order 2 to Order 0 (q2/q0) — reveals how evenly clones are distributed: a ratio near 1.0 means all clones are similar in size, while a low ratio indicates dominance by a few large clones. Recovered individuals showed the steepest negative slope (median q2/q0 = 0.49), followed by Severe (0.55) and Moderate (0.60), indicating that despite having high absolute diversity, recovered repertoires are dominated by a smaller number of expanded clones. COVID Naive (0.61) and Healthy (0.69) showed less steep declines, reflecting somewhat more even clone size distributions. Individuals with mild disease had the flattest profiles (q2/q0 = 0.90), but this reflects the very small number of clones detected in this group (median q0 = 4) rather than true evenness — with so few clones, there is little room for dominance to emerge.

![Figure 2. Diversity profiles by disease stage. Each thin line represents one individual's Hill number profile from Order 0 to Order 2; thick lines show group medians. A steep drop indicates dominance by a few large clones; a gradual decline indicates a more even clone size distribution.](../immunedb_STATS_API/clonal_analysis/plots/18_diversity_profiles.png){ width=100% }

## Expanded Clones and Clone Size Distribution

**Expanded clone count.** We examined the number of expanded clones at three expansion thresholds: 100, 50, and 20 unique sequences per clone (**Figure 3**). At the most stringent threshold (>100 unique sequences), only 60 of 94 individuals (64%) had any expanded clones. The disease groups were most affected: only 9 of 27 individuals with severe disease, 3 of 9 with moderate disease, and 2 of 41 with mild disease retained any expanded clones at this level. In contrast, 11 of 12 recovered, all 8 COVID-naive, and 8 of 9 healthy individuals still had ≥100-sequence clones. Lowering the threshold to >50 unique sequences improved representation to 78 of 94 individuals (83%), but 4 individuals with severe and 11 with mild disease still had no expanded clones at this cutoff, while COVID-naive and healthy individuals consistently retained multiple expanded clones. At the lowest threshold (>20 unique sequences), all 94 individuals had at least one expanded clone, confirming that clonal expansion is a universal feature of BCR repertoires regardless of disease status. Even at this inclusive threshold, the number of expanded clones varied dramatically: COVID-naive individuals had a median of 521 expanded clones (range 328–920), while individuals with mild disease had a median of only 9 (range 0–2,117), and those with severe disease had a median of 34 (range 1–2,484). This threshold-dependent pattern shows that while low-level expansion is ubiquitous, sustained large-scale clonal expansion is more characteristic of non-disease repertoires, and that disease groups — particularly Severe and Mild — lose representation rapidly as the expansion threshold increases.

![Figure 3. Number of expanded clones per individual at three expansion thresholds. Left: threshold >20 unique sequences (94/94 individuals = 100%). Center: threshold >50 (78/94 = 83%). Right: threshold >100 (60/94 = 64%). Red annotations indicate individuals with zero expanded clones at each threshold. Colors as in Figure 1. Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/19_expanded_clones_threshold.png){ width=100% }

**Clone size distribution.** The clone size distribution analysis (**Figure 4**) showed two complementary views. **Figure 4A** displayed the distribution of all individual clone sizes (log-transformed) as violin plots with overlaid boxplots, revealing that clone sizes were broadly similar across categories but with longer tails in the Severe, Recovered, COVID Naive, and Healthy groups. **Figure 4B** showed the median clone size per individual, which was significantly higher in COVID Naive individuals compared to the disease groups (Kruskal-Wallis p = 0.019).

![Figure 4. Clone size distribution by disease stage. (A) violin plots with overlaid boxplots of all individual clone sizes (log-transformed), showing the shape of each group's distribution. (B) median clone size per individual by disease category, with Kruskal-Wallis p-value annotated. Each dot represents one individual.](../immunedb_STATS_API/clonal_analysis/plots/20_clone_size_distribution.png){ width=100% }

**Clone size: unique sequences versus copies.** To understand the relationship between the two measures of clone size, we compared the number of unique sequences per clone with the total number of copies (raw reads) per clone (**Figure 5**). The per-clone scatter plot (**Figure 5A**) shows a strong positive correlation between unique sequences and copies across all disease categories on a log-log scale, with most clones falling near the diagonal reference line. This confirms that both measures capture the same underlying biological signal. However, some clones — particularly from disease groups — fall well above the diagonal, indicating clones with disproportionately high copy counts relative to their number of unique sequences. The per-subject median view (**Figure 5B**) reveals that disease groups (Severe, Moderate, Mild) tend to cluster with higher median copy counts relative to median unique sequences compared to non-disease groups (Recovered, COVID Naive, Healthy), which cluster more tightly near the diagonal. This suggests that clones in disease repertoires are more frequently re-sequenced (higher redundancy), which could reflect either biological amplification or technical sequencing bias.

![Figure 5. Clone size: copies versus unique sequences by disease stage. (A) scatter plot of copies (raw reads) vs. unique sequences per clone across all individuals and disease categories (log-log scale); dashed line marks y=x. (B) per-individual median copies vs. median unique sequences, colored by disease category. Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/26_copies_vs_unique.png){ width=100% }

**Clonal concentration (Top X).** Querying the topX_clone_size_copies endpoint showed the fraction of total sequence copies accounted for by the top 10, 100, and 1000 clones (**Figure 6**), revealing the degree of oligoclonal dominance in each disease category.

![Figure 6. Clonal concentration: fraction of total sequence copies accounted for by the top 10, top 100, and top 1000 clones per individual, shown as stacked bars by disease stage. Y-axis: cumulative fraction of total copies (0–1). Higher values indicate greater oligoclonal dominance. Each bar represents one individual. Colors distinguish disease categories (colors as in Figure 1). Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/02_topX_stacked_by_disease.png){ width=100% }

## CDR3 Length and Mutation Analysis

Having established the clone size distribution and the expansion threshold of ≥20 unique sequences — chosen because at this threshold 100% of individuals across all disease categories retain at least one expanded clone, making it suitable for cross-group comparison — we next examined CDR3 length and somatic hypermutation patterns across disease categories. IS-API's CDR3 and mutation endpoints enable analysis at multiple levels: all clones per individual, expanded versus non-expanded clones, and the top 10 most expanded clones.

**CDR3 length distribution: all clones versus expanded clones.** We examined CDR3 length per individual across all clones (**Figure 7**) and expanded clones only (**Figure 8**, ≥20 unique sequences). Both figures share the same y-axis scale to allow direct visual comparison; the wider range is driven by expanded-clone outliers (see below). Across all clones (**Figure 7**), mean CDR3 length differed significantly across disease categories (Kruskal-Wallis p < 0.0001): Individuals with moderate disease had the longest mean CDR3 lengths (median 17.8 AA), while those with mild disease and healthy individuals had shorter CDR3 (median ~17.0 AA). The standard deviation of CDR3 length also varied (p = 0.0009), with individuals with severe disease showing the highest within-individual variability. When restricted to expanded clones (**Figure 8**), the disease-stage differences became more pronounced (Kruskal-Wallis p = 0.0008 for both mean and SD): healthy individuals showed dramatically shorter expanded-clone CDR3 lengths (median 14.4 AA) compared to disease groups (16–18 AA), and individuals with severe disease had the highest expanded-clone CDR3 variability (median SD ~4.5 AA), suggesting a more structurally diverse set of expanded clones during acute infection. Note that the shared y-axis with **Figure 7** reveals how much wider the CDR3 length distribution becomes when focusing on expanded clones — individual outliers with very short or very long mean CDR3 lengths stretch the range well beyond what is observed across all clones, where repertoire averaging narrows the spread.

![Figure 7. CDR3 amino acid length distribution — all clones by disease stage. (A) mean CDR3 length per individual. (B) standard deviation of CDR3 length per individual. Each dot represents one individual; boxplots show median and IQR. Kruskal-Wallis p-values annotated. Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/22a_cdr3_length_all_clones.png){ width=100% }

![Figure 8. CDR3 amino acid length distribution — expanded clones (≥20 unique sequences) by disease stage. (A) mean CDR3 length per individual. (B) standard deviation of CDR3 length per individual. Each dot represents one individual; boxplots show median and IQR. Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/22b_cdr3_length_expanded.png){ width=100% }

**Selection pressure and mutation profile (NS/S ratios).** To analyze selection pressure and mutation burden, we examined NS/S ratios separately for CDR and framework (FW) regions across all clones (**Figure 9**) and stratified by clone expansion status (**Figure 10**). The NS/S ratios are computed by the API as the average of per-clone ratios within each individual — that is, each clone's NS/S ratio is calculated first (using `AVG(NS / NULLIF(S, 0))` across clones), then averaged — rather than dividing the average NS count by the average S count, which would bias the ratio toward clones with high absolute mutation counts.

![Figure 9. Somatic hypermutation — all clones by disease stage. (A) CDR NS/S ratio per individual. (B) FW NS/S ratio (same y-axis scale as A). (C) average NS and S mutation counts as stacked bars for CDR and FW regions. Boxplots show median and IQR; dashed line marks NS/S = 2.0 (neutral expectation). Kruskal-Wallis p-values annotated. Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/21a_mutations_all_clones.png){ width=100% }

Under neutral evolution, the expected NS/S ratio is approximately 2, reflecting the roughly 2:1 ratio of possible nonsynonymous to synonymous mutations in the genetic code; values above 2 indicate positive selection and values below 2 indicate purifying selection. Across all clones (**Figure 9A–C**), CDR NS/S ratios were above the neutral expectation in most disease categories, confirming positive selection in antigen-binding regions: Mild (median 2.93) and Severe (2.89) showed the strongest positive selection, while Recovered (2.59), COVID Naive (2.62), and Healthy (2.68) showed moderate positive selection. Moderate cases were the exception, with CDR NS/S below neutral (1.79), indicating purifying selection in CDR regions. The Kruskal-Wallis test revealed significant differences across groups (p < 0.001). Among disease states, Moderate CDR NS/S was significantly lower than Mild (p < 0.001) and Recovered (p < 0.01); compared to the non-diseased groups, Moderate was also lower than both COVID Naive and Healthy. FW NS/S ratios (**Figure 9B**) were below the neutral expectation in all groups, consistent with purifying selection maintaining framework structural integrity. FW NS/S also showed significant differences across disease categories (Kruskal-Wallis p = 0.002), with Healthy (1.30) and COVID Naive (1.23) showing the strongest purifying selection and Recovered individuals (1.89) having significantly higher FW NS/S (closer to neutral) than Healthy (p < 0.001). **Figure 9C** shows that absolute mutation counts were highest in Recovered individuals, consistent with extensive somatic hypermutation in memory B cells following infection and vaccination.

![Figure 10. Somatic hypermutation — expanded vs. unexpanded by disease stage. (A) CDR NS/S ratio and (B) FW NS/S ratio (same y-axis scale as A) comparing expanded clones (≥20 unique sequences, solid boxes) versus unexpanded clones (faded boxes). Boxplots show median and IQR; individual dots represent one subject. Dashed line marks NS/S = 2.0 (neutral expectation). Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/21b_mutations_expanded_vs_rest.png){ width=100% }

When stratified by clone expansion status (**Figure 10**), an interesting divergence emerged between disease categories. In individuals with severe (expanded CDR NS/S: 2.12 vs. unexpanded: 2.85) and mild (2.50 vs. 2.94) disease, expanded clones showed *lower* CDR NS/S ratios than unexpanded clones, suggesting that clonal expansion in active disease may outpace the accumulation of positively selected replacement mutations. In contrast, recovered (3.17 vs. 2.53), COVID-naive (3.38 vs. 2.50), and healthy (3.11 vs. 2.70) individuals showed the opposite pattern — expanded clones had *higher* CDR NS/S ratios — consistent with affinity-matured memory clones that have undergone strong positive selection. The FW NS/S ratios revealed an even more striking pattern: expanded clones in COVID-naive (expanded: 2.99 vs. unexpanded: 1.14) and healthy (2.83 vs. 1.07) individuals had dramatically higher FW NS/S ratios than unexpanded clones, while in individuals with severe disease the ratios were nearly identical (1.81 vs. 1.81). This suggests that in non-diseased individuals, expanded clones tolerate more framework replacement mutations, possibly reflecting longer-lived memory clones that have accumulated structural adaptations over time.
...
![Figure 11. V gene usage heatmap by disease stage. Each column represents one individual, ordered by hierarchical clustering (Ward linkage, Euclidean distance) of V gene usage profiles; each row represents one V gene segment, sorted alphanumerically. Color intensity indicates the frequency (%) of each V gene within that individual's repertoire. The disease color bar at top indicates each individual's disease category. Only V genes present at ≥1% frequency in ≥85% of individuals are shown (16 genes). Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/23_v_gene_usage_heatmap.png){ width=100% }

V genes were included if they were present at 1% or higher frequency in at least 85% of individuals, resulting in 16 V genes that met this criterion. V gene rows are sorted alphanumerically (IGHV1 through IGHV5), while individuals (columns) are ordered by hierarchical clustering (Ward method, Euclidean distance) of their V gene usage profiles, with the disease color bar at top indicating each individual's disease category.

The heatmap reveals that a small set of V genes — IGHV4-34, IGHV4-39, IGHV4-59, and IGHV3-23|3-23D — dominates the repertoire with frequencies consistently above 5–10% across all disease categories, while the remaining 12 genes show lower and more variable usage (**Figure 11**).

To test whether disease states could be distinguished by V gene usage, we also clustered individuals (columns) by their V gene usage profiles using hierarchical clustering (Ward method, Euclidean distance) and visualized the result as a dendrogram (**Figure 12**). Disease categories were interleaved throughout the dendrogram, with no clear separation of disease states into distinct branches. Individuals from different disease categories were frequently nearest neighbors in the clustering, confirming that V gene usage profiles do not discriminate between disease states. However, the 85% presence threshold used to select V genes for this analysis means that only broadly shared genes were included; V genes that are preferentially expanded or enriched specifically in diseased individuals — but rare or absent in healthy controls — would be excluded by this criterion. Disease-associated V gene usage shifts, if they exist, may therefore involve lower-frequency genes not captured in this analysis. This observation of stability among the broadly shared V genes is consistent with the known stability of V gene usage across individuals and conditions [37].

![Figure 12. Dendrogram of individuals clustered by V gene usage profiles. Hierarchical clustering (Ward linkage, Euclidean distance) of 106 blood individuals based on the frequency of 16 V genes. Leaf labels are colored by disease category. Disease states are interleaved throughout the tree, indicating that V gene usage does not cluster by disease stage.](../immunedb_STATS_API/clonal_analysis/plots/23b_v_gene_individual_dendrogram.png){ width=100% }

## Potential Confounders: Age, Gender, and Sequencing Method
...
![Figure 13. Clonal metrics versus age by disease stage. (A) clone count (log scale) vs. age. (B) mean CDR3 length vs. age. (C) CDR NS/S ratio vs. age. Each dot represents one individual, colored by disease category. Three CD3 healthy individuals (H3, H4, H8) excluded due to missing age/sex metadata.](../immunedb_STATS_API/clonal_analysis/plots/24_metrics_vs_age.png){ width=100% }

**Age (**Figure 13**).** Scatter plots of each metric against age showed no strong linear trends within any disease category. Clone count (A, log scale) showed substantial variation within age groups but no systematic increase or decrease with age. Mean CDR3 length (B) was similarly independent of age. The CDR NS/S ratio (C) showed no age-dependent trend. Previous studies have reported that B cell repertoire diversity decreases with age and that overall somatic hypermutation counts can increase with cumulative antigen exposure [50, 51]; however, those findings concern absolute mutation levels and clonal diversity rather than the NS/S ratio, which reflects selection pressure. Since both replacement and synonymous mutations accumulate together, the NS/S ratio is not expected to show the same age-dependent increase as absolute mutation counts. The absence of clear age-dependent trends in any of the three metrics in our data, combined with the relatively narrow age ranges within disease categories and limited sample sizes, suggests that age is not a major confounder in our disease-stage comparisons, though subtle age effects cannot be ruled out.

![Figure 14. Clonal metrics by gender and disease stage. (A) clone count (log scale). (B) mean CDR3 length. (C) CDR NS/S ratio. Boxplots compare male and female individuals within each disease category.](../immunedb_STATS_API/clonal_analysis/plots/25_metrics_vs_gender.png){ width=100% }

**Gender (**Figure 14**).** Boxplots comparing male and female individuals within each disease category showed no systematic differences in clone count (A, log scale), mean CDR3 length (B), or CDR NS/S ratio (C). The lack of sex-based differences is consistent with the literature for peripheral blood BCR repertoires, though our sample sizes within each sex-disease combination are limited, and subtle differences cannot be ruled out.

![Figure 15. Clonal metrics by sequencing method and disease stage. (A) Clone count (log scale). (B) Mean CDR3 length. (C) CDR NS/S ratio. Within each disease category, solid boxes represent 5'RACE + UMI studies (CD1, CVX1, CVX2) and hatched boxes represent multiplex PCR studies (CD2, CD3, HC1). Disease categories with only one sequencing method (Moderate: PCR only; COVID Naive: 5'RACE only; Healthy: PCR only) show a single box. Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/26_metrics_vs_seq_method.png){ width=100% }

**Sequencing method (**Figure 15**).** Because the six studies used different library preparation methods — 5'RACE with UMI error correction (CD1, CVX1, CVX2) versus multiplex PCR without UMIs (CD2, CD3, HC1) — and these methods are unevenly distributed across disease categories (see Metadata Figure 5), sequencing method is a potential confounder. We assessed this in two complementary ways: (1) comparing methods within disease categories where both are represented, and (2) comparing disease categories within each method to check whether the biological patterns hold.

*Within-disease comparison.* In the Severe group (n=11 5'RACE, n=15 PCR), clone counts were notably higher in PCR-sequenced individuals, likely reflecting the higher read depth and broader primer coverage of the CD2/CD3 studies rather than a biological difference. However, mean CDR3 length and CDR NS/S ratio showed overlapping distributions between methods within both the Severe and Recovered groups, suggesting that these metrics are less sensitive to sequencing methodology.

*Across-disease comparison within each method.* Critically, when restricting the analysis to a single sequencing method, the same disease-stage patterns observed in the full cohort were reproduced. Among 5'RACE+UMI studies only (CD1, CVX1, CVX2), individuals with mild disease still had the lowest clone counts, individuals with severe disease still showed high CDR NS/S ratios, and COVID-naive and recovered individuals still had higher clone counts — mirroring the full-cohort findings. Similarly, among multiplex PCR studies only (CD2, CD3, HC1), the Moderate group still had lower CDR NS/S ratios than the Severe group, and healthy individuals still had high clone counts — again consistent with the full-cohort analysis. Because the disease-stage differences persist even when sequencing method is held constant, this confirms that the biological patterns reported in earlier sections are not artifacts of methodological differences between studies.

Three disease categories — Moderate (PCR only), COVID Naive (5'RACE only), and Healthy (PCR only) — were each sequenced with a single method, making it impossible to fully disentangle disease effects from method effects for these specific groups. This confound is an inherent limitation of the cross-study design and should be considered when interpreting differences involving these categories.

## Sex-Stratified Analysis

By combining disease_stage and sex metadata filters in a single query, IS-API enabled examination of NS/S ratios stratified by sex within each disease category. We examined both CDR and FW NS/S ratios separately for all clones (**Figure 16**) and for expanded clones only (**Figure 17**), demonstrating the API's ability to perform intersectional queries across multiple metadata dimensions. Both figures share the same y-axis scale for direct comparison.

Across all clones (**Figure 16**), neither CDR (A) nor FW (B) NS/S ratios showed systematic differences between male and female individuals within any disease category, consistent with the gender analysis in **Figure 14**. When restricted to expanded clones (**Figure 17**), the same pattern held — no consistent sex-based differences in selection pressure were observed in either CDR or FW regions. The expanded-clone FW NS/S ratios were notably higher than the all-clone FW ratios (compare **Figure 16B** vs **Figure 17B**), particularly in Recovered, COVID Naive, and Healthy groups, consistent with the expanded-clone FW NS/S elevation observed in **Figure 10**. The absence of sex-based differences in both views and both regions reinforces the conclusion that disease stage, rather than sex, is the primary driver of NS/S ratio variation in this dataset.

![Figure 16. NS/S ratios — all clones by sex and disease stage. (A) CDR NS/S ratio. (B) FW NS/S ratio (same y-axis scale as A). Male (solid) and female (hatched) individuals compared within each disease category. Each dot represents one individual; boxplots show median and IQR. Dashed line marks NS/S = 2.0 (neutral expectation). Blood samples only. Three CD3 healthy individuals (H3, H4, H8) excluded due to missing sex metadata.](../immunedb_STATS_API/clonal_analysis/plots/28_nss_all_clones_by_sex.png){ width=100% }

![Figure 17. NS/S ratios — expanded clones (≥20 unique sequences) by sex and disease stage. (A) CDR NS/S ratio. (B) FW NS/S ratio (same y-axis scale as A). Male (solid) and female (hatched) individuals compared within each disease category. Each dot represents one individual; boxplots show median and IQR. Dashed line marks NS/S = 2.0 (neutral expectation). Blood samples only.](../immunedb_STATS_API/clonal_analysis/plots/29_nss_expanded_by_sex.png){ width=100% }

## Within-Individual Cross-Tissue Analysis

A key capability of IS-API is the ability to query and compare clonal data across multiple tissue types and across different studies simultaneously. To demonstrate this, we used two multi-tissue studies hosted on separate ImmuneDB database instances: HC1 (healthy donors) with eight tissue types — PBL (peripheral blood), BM (bone marrow), SPL (spleen), Lung, Colon, Ileum, Jejunum, and MLN (mesenteric lymph node) — and GT1 (gut transplant recipients) with a broader tissue panel including PBMC (peripheral blood, equivalent to PBL), Bone Marrow, Spleen, Colon, Ileum, Jejunum, Duodenum, MLN, and corresponding allograft tissues.

We first used IS-API to query clone counts across all eight HC1 tissues for each healthy donor individually (**Figure 18**), illustrating within-study, within-individual tissue comparisons. We then leveraged IS-API's cross-study capability to compare clone size distributions between an HC1 and a GT1 individual in shared gut tissues (**Figure 19**), and to compare median clone sizes across all subjects from both studies by tissue (**Figure 20**). These cross-study, cross-tissue comparisons — querying two independent databases through a single API — would be difficult to perform without IS-API's unified interface.

![Figure 18. Clone count per sample per tissue for each HC1 healthy donor. Each subplot shows one individual; bars represent the number of clones detected in each of the eight HC1 tissues (PBL, BM, SPL, Lung, Colon, Ileum, Jejunum, MLN), normalized by the number of biological samples sequenced from that tissue. A clone present in multiple tissues is counted once in each tissue it appears in; its size in a given tissue reflects only the sequences found in that tissue, not the total clone size across all tissues. Y-axis is uniform across all subplots.](../immunedb_STATS_API/clonal_analysis/plots/14_within_subject_clone_count.png){ width=100% }

![Figure 19. Comparison of tissue clone size in healthy (green, HC1 donor D207) and post-transplant (grey, GT1 donor Pt20_R) gut tissue (Colon, Jejunum, and Mesenteric Lymph Node). Clone size is the number of unique sequences per clone within each tissue; for clones shared across tissues, only the sequences found in the displayed tissue are counted. Red diamonds indicate the mean of each distribution; n = number of clones.](../immunedb_STATS_API/clonal_analysis/plots/15_within_subject_clone_size_cross_study.png){ width=100% }

![Figure 20. Median clone size per subject by tissue — HC1 (green, healthy) vs. GT1 (grey, gut transplant). Each dot represents one individual's median clone size in that tissue. Horizontal bars indicate the group median across subjects. Only tissues present in both studies are shown. Y-axis: log scale.](../immunedb_STATS_API/clonal_analysis/plots/16_within_subject_tissue_lines.png){ width=100% }

Because different tissues were sampled at different depths — for example, donor D207 had 43 PBL samples but only 20 Jejunum samples, while donor D145 had 6 samples uniformly across all tissues — we normalized clone counts by the number of biological samples per tissue (**Figure 18**). Even after this normalization, clone counts vary substantially across tissues within the same healthy donor, with peripheral blood (PBL) typically harboring the most clones per sample. The cross-study gut tissue comparison (**Figures 19–20**) revealed differences in clone size distributions between healthy donors and post-transplant patients in shared gut tissues (Colon, Jejunum, MLN), consistent with the distinct immunological environments of native versus post-transplant gut mucosa. The ability to make such comparisons across independently built databases — without manual data harmonization — demonstrates the practical utility of IS-API for multi-study immune repertoire analysis.
...
**Clonal diversity decreases during active disease.** The Hill number analysis showed that individuals with active COVID-19 (Mild, Moderate, and Severe) had substantially lower clonal diversity than COVID-naive and recovered individuals across all three diversity orders (**Figure 2**). Individuals with mild disease showed the lowest diversity, followed by those with severe and moderate disease. This likely reflects the focusing of the B cell repertoire during an active immune response — antigen-driven clonal expansion leads to the dominance of a few clones, reducing overall diversity. Similar observations of reduced diversity during active immune responses have been reported in individual studies [28, 32].

Notably, recovered individuals showed higher diversity than healthy controls, and COVID-naive individuals showed the highest diversity overall. The elevated diversity in recovered individuals may reflect repertoire reconstitution following infection — as the acute clonal expansion subsides, the repertoire rebounds with a broadened pool of memory B cells generated during the immune response. The high diversity in COVID-naive individuals likely reflects an unperturbed, antigen-naive repertoire that has not undergone the focusing effects of a recent infection.

**NS/S ratios indicate ongoing selection in disease states.** Under neutral evolution, the expected NS/S ratio is approximately 2, reflecting the roughly 2:1 ratio of possible nonsynonymous to synonymous mutations in the genetic code. CDR NS/S ratios above 2 therefore indicate positive selection, while ratios below 2 indicate purifying selection. Most disease categories showed CDR NS/S ratios above the neutral expectation (Severe: 2.89, Mild: 2.93, Recovered: 2.59, COVID Naive: 2.62, Healthy: 2.68), confirming positive selection in CDR regions consistent with antigen-driven affinity maturation. Individuals with moderate disease were the exception, with CDR NS/S below neutral (median 1.79). Among disease states, this was notably lower than Severe (2.89) and Mild (2.93), and also lower than the non-diseased groups (COVID Naive: 2.62, Healthy: 2.68). This suggests purifying rather than positive selection in CDR regions in the Moderate group, which may indicate less efficient germinal center responses or a predominance of recently activated B cells that have not yet undergone extensive affinity maturation. FW NS/S ratios were below the neutral expectation in all groups, consistent with purifying selection maintaining framework structural integrity. Healthy (1.30) and COVID Naive (1.23) individuals showed the strongest FW purifying selection, while disease groups showed FW NS/S closer to neutral (~1.8), suggesting that disease-driven perturbation may relax the purifying constraint on framework regions. The expanded vs. unexpanded clone analysis revealed a particularly interesting pattern: in active disease (Severe, Mild), expanded clones had lower CDR NS/S ratios than unexpanded clones, suggesting rapid expansion outpacing affinity maturation, while in Recovered, COVID Naive, and Healthy individuals the reverse was true — expanded clones showed higher CDR NS/S ratios, consistent with well-matured memory B cells.

**CDR3 length varies with disease severity.** Individuals with moderate disease had the longest median CDR3 lengths (17.82 AA), followed by COVID Naive (17.63) and Recovered (17.57), while Severe (17.36), Healthy (17.05), and Mild (16.90) had shorter CDR3 lengths (**Figure 7**). Beyond differences in central tendency, the variability of CDR3 lengths differed substantially between groups. Healthy individuals showed the widest spread (std = 1.09 AA, range 14.90–18.23), while disease groups were considerably more constrained — particularly Moderate (std = 0.34) and Mild (std = 0.46). COVID Naive individuals showed the tightest distribution (std = 0.25). This suggests that active or recent infection may narrow the CDR3 length distribution, possibly through selective expansion of clones with CDR3 lengths in a particular range, whereas healthy repertoires retain a broader baseline diversity of CDR3 lengths. The longer CDR3 lengths in individuals with moderate disease and COVID-naive individuals are consistent with the known association between longer CDR3 loops and broadly neutralizing antibody responses [11] — longer CDR3 regions can form more complex antigen-binding structures, potentially enabling recognition of diverse viral epitopes. The shorter CDR3 lengths in individuals with mild disease may reflect a less mature or less diversified response.

**V gene usage is relatively stable across disease states.** The V gene usage heatmap (**Figure 11**) revealed that a small set of high-frequency genes — IGHV3-23, IGHV4-59, IGHV4-34, and IGHV4-39 — dominates the repertoire across all conditions, while the remaining 12 genes show lower and more variable usage. Hierarchical clustering of individuals by their V gene usage profiles showed no separation of disease categories into distinct branches; individuals from different disease groups were frequently nearest neighbors in the clustering. The dendrogram (**Figure 12**) confirmed this pattern, with disease categories interleaved throughout. This stability is consistent with findings from large-scale repertoire studies showing that V gene usage is primarily determined by germline genetics rather than by antigen exposure or disease state [37, 6]. While specific antibody lineages targeting SARS-CoV-2 epitopes may preferentially use certain V genes [38], these represent a small fraction of the total repertoire and may not be detectable at the level of overall V gene frequency. Additionally, because the analysis included only V genes present at ≥1% frequency in at least 85% of individuals, disease-specific V gene enrichments involving lower-frequency genes would not have been captured.
...
Several limitations should be acknowledged. First, the cohort is heterogeneous in terms of sequencing platforms, library preparation methods, and sample processing across studies. While IS-API standardizes the analysis, upstream differences could introduce systematic biases. Second, the clone definition (based on shared V gene, J gene, and CDR3 similarity) may not perfectly capture biological clones across different analysis pipelines. Third, the threshold of 20 or more unique sequences for clone inclusion means that rare clones are excluded, which may bias diversity estimates. Fourth, the sample sizes within some disease categories (particularly Moderate with n=9 and COVID Naive with n=8) are small, limiting statistical power. Fifth, the Recovered group includes both naturally recovered and vaccine-recovered individuals, which may have different repertoire characteristics. Future work should aim to analyze these subgroups separately with larger cohorts. Sixth, both the Healthy control group (from CD3 and HC1) and the COVID Naive group (from CVX1) were classified based on clinical metadata provided in the original studies, and serological verification of prior asymptomatic SARS-CoV-2 infection was not uniformly available. While the HC1 organ donors were sampled before the COVID-19 pandemic (published 2017), making prior SARS-CoV-2 exposure impossible, the CD3 healthy controls and the CVX1 COVID Naive individuals were contemporaneous with the pandemic and could potentially have had undetected prior infections. This should be considered when interpreting repertoire differences involving the Healthy and COVID Naive groups.
...
50. Dunn-Walters, D. K., & Ademokun, A. A. (2010). B cell repertoire and ageing. *Current Opinion in Immunology*, 22(4), 514-520.

51. Gibson, K. L., Wu, Y. C., Barnett, Y., et al. (2009). B-cell diversity decreases in old age and is correlated with poor health status. *Aging Cell*, 8(1), 18-25.
